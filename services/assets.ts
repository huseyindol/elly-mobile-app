...
import type { AssetResponse, AssetListResponse, AssetPagedResponse, SubFoldersResponse } from '../types/asset';
...
export const assetsService = {
  ...
  getSubFolders: () =>
    apiClient.get<SubFoldersResponse>('/assets/sub-folders'),
  getById: (id: number | string) =>
    apiClient.get<AssetResponse>(`/assets/id/${id}`),
  ...
  searchBySubFolderAndName: (subFolder: string, name: string, params?: ListParams) =>
    apiClient.get<AssetPagedResponse>(`/assets/${encodeURIComponent(subFolder)}/${encodeURIComponent(name)}/paged`, { params }),
  ...
  uploadMulti: (files: Array<{ uri: string; type: string; name: string }>, subFolder?: string) => {
    const formData = new FormData();
