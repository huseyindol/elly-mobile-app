    ...
    <>
      ...
      >
        ...
        <FormField
          ...
          name="contentType"
          label="İçerik Türü"
          placeholder="örn: hero, slider, card"
        />
            ...
            render={({ field }) => (
              <View style={styles.stepperRow}>
                <Text
                  ...
                  onPress={() => field.onChange(Math.max(0, field.value - 1))}
                >
                  −
                </Text>
                <Text style={styles.stepperValue}>{field.value}</Text>
                <Text style={styles.stepper} onPress={() => field.onChange(field.value + 1)}>
                  +
                </Text>
              </View>
            )}
      ...
      >
        <Text style={[styles.cardTitle, { color: colors.text }]}>Metadata (JSON)</Text>
        <Text style={styles.metaHint}>
          Geçerli JSON formatında giriniz. Örn: {`{"key":"value"}`}
        </Text>
        <FormField
          control={control}
          name="metadata"
